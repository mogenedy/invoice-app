...
<div class="content-wrapper">
    ...
    <div class="container">
        ...
        <form id="invoice-form" method="POST">
            ...
            <div class="form-group">
                <label for="client_id">Client Address:</label>
                <select class="form-control" id="client_id" name="client_address" required>
                    <option value="">Select Client</option>
                    @foreach($clients as $client)
                        <option value="{{ $client->id }}" >{{ $client->address }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div id="items-container">
                <div class="item-group">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="description_1" name="items[0][description]" required>
                    </div>
                    <div class="form-group">
                        <label for="quantity_1">Quantity:</label>
                        <input type="number" class="form-control" id="quantity_1" name="items[0][quantity]" min="1" required>
                    </div>
                    <div class="form-group">
                        <label for="price_per_unit_1">Price per unit:</label>
                        <input type="number" class="form-control" id="price_per_unit_1" name="items[0][price_per_unit]" step="0.01" min="0" required>
                    </div>
                    <div class="form-group">
                        <label for="total_1">Total:</label>
                        <input type="number" class="form-control" id="total_1" name="items[0][total]" step="0.01" min="0" readonly>
                    </div>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function() {
            $('#client_id').change(function() {
                ...
                var clientAddress = selectedOption.data('address');
